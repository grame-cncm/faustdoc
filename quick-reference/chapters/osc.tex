...
\code{-osc} option when building the application with the appropriate \code{faust2xxx} command. The following table (Table \ref{tab:oscarch}) lists the \faust architectures that provide OSC support. 
...
Note that if a \code{ declare name \"Foo\"; } line is present in the DSP program, \code{Foo} will be used as the OSC root name, otherwise the DSP filename will be used instead.
...
\item \lstinline'-desthost host' sets the destination host for the messages sent by the application. Note that the destination address can be changed with the first incoming message: the first received packet from another host sets the destination address to that host.
...
Here is, as an example, \code{mix4.dsp}, a very simplified monophonic audio mixer with four inputs and one output. For each input we have a mute button and a level slider:
\begin{lstlisting}
