...
But since all voices would always be computed, this approach could be too CPU costly for simpler or more limited needs. In this case describing a single voice in a \faust DSP program and externally combining several of them with a special  \textit{polyphonic instrument aware} architecture file is a better solution. Moreover, this special architecture file takes care of dynamic voice allocations and control MIDI messages decoding and mapping.

\section{Polyphonic ready DSP code}

By convention \faust architecture files with polyphonic capabilities expect to find control parameters named \textit{freq}, \textit{gain} and \textit{gate}.  The metadata \code{declare nvoices "8";}   kind of line with a desired value of  voices can be added in the source code.

In the case of MIDI control, the \textit{freq} parameter (which should be a frequency) will be automatically computed from MIDI note numbers, \textit{gain} (which should be a value between 0 and 1) from velocity and \textit{gate} from \textit{keyon/keyoff} events. Thus, gate can be used as a trigger signal for any envelope generator, etc.
...
\item  the \textit{faust/dsp/poly-dsp.h} file contains the definition of the \code{mydsp_poly} class used to wrap the DSP voice into the polyphonic architecture. This class maintains an array of \code{dsp}  type of objects, manage dynamic voice allocations, control MIDI messages decoding and mapping, mixing of all running voices, and stopping a voice when its output level decreases below a given threshold.
...
The \code{mydsp_poly} class is also ready for MIDI control and can react to \textit{keyon/keyoff} and \textit{pitchwheel} messages. Other MIDI control parameters can directly be added in the DSP source code. 
...
A new \code{auto} parameter to be used in \textit{faust2xx} script has been defined, as in the \code{faustcaqt inst.dsp -effect auto} line for example.
