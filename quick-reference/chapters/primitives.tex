...
\begin{itemize} 
\item \lstinline'A' is the number of input signals, 	as a constant numerical expression 
\item \lstinline'B' is the number of output signals,  as a constant numerical expression 
\item \lstinline'a,b / (a,b)' is an input/output pair,  as constant numerical expressions 
\end{itemize}
...
\begin{itemize} 
\item two inputs: the sound number (as a integer between 0 and 255 as a \textit{constant numerical expression}), and the read index in the sound (which will access the last sample of the sound if the read index is greater than the sound length)
...
Most of them do automatic parameter promotion to \texttt{float} type, except \lstinline'min' and \lstinline'max' that use \texttt{int} type when both of their parameters are \texttt{int}, and use \texttt{float} otherwise.
