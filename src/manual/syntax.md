...
Faust is a lexically scoped language. The meaning of a Faust expression is determined by the context where it is defined (its lexical environment) rather than by the context where it is used.

To keep their original meaning, Faust expressions are bounded to their lexical environment in structures called *closures*. The following constructions allow explicit creation and access to these lexical environments, providing statements that create a modified local environemnt in order to reuse existing code and promote modular design.
...
```
pi = environment{pi = 3.14159; e = 2.718; /* ... */ }.pi
```
...
```
/* lhs ... */ component("karplus32.dsp") : component("freeverb.dsp") /* ... rhs */
```
...
```
/* lhs ... */ component("freeverb.dsp")[foo(x) = /* ... */;] /* ... rhs */
};
```
