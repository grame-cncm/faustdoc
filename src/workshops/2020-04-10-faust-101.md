...
But, just as in `(2*3)+7` the parentheses are not really necessary because multiplication takes precedence over addition, one could write directly `process = _,0.1 : *;` without the partentheses because parallel composing takes precedence over sequential composing as indicated in the previous table.
