...
All classes needing to use this synchronization mechanism will have to subclass the `GUI` class, which keeps all of them at runtime in a static global `GUI::fGuiList` variable. This is the case for the previously used `GTKUI` and `OSCUI` classes. Note that when using the `GUI` class, **the 2 following static global variables have to be defined in the code,** (once in one `.cpp` file in the project) like in this code example:
