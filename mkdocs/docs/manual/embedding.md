...
- next, the `createDSPInstance` function (corresponding to the `new className` of C++) instantiates a `llvm_dsp` pointer to be used through its interface, connected to the audio chain and controller interfaces. When finished, `delete` can be used to destroy the dsp instance, or possibly using [smart pointers](https://en.cppreference.com/w/cpp/memory.html#Smart_pointers). Note that an instance internally needs to access its associated factory during its entire lifetime
...
After the DSP factory has been compiled, the application or plugin may want to save/restore it in order to save Faust to wasm bytecode compilation at next use. To get the internal factory bytecode and save it, two functions are available:
  ...
  - `writeWasmDSPFactoryToBitcode` allows to get the wasm binary code as a string
  - `writeWasmDSPFactoryToBitcodeFile` allows to save the wasm binary code in a file
